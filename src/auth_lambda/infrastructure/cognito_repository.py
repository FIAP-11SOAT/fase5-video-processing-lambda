"""
Cognito Repository - Implementation of IAuthRepository using AWS Cognito
"""
import os
import boto3
...
class CognitoRepository(IAuthRepository):
    """AWS Cognito implementation of authentication repository"""
    
    def __init__(self, user_pool_id: Optional[str] = None, client_id: Optional[str] = None):
        """
        Initialize Cognito repository
        
        Args:
            user_pool_id: Cognito User Pool ID (defaults to env var)
            client_id: Cognito App Client ID (defaults to env var)
        """
    ...
    def authenticate(self, username: str, password: str) -> AuthToken:
        try:
            ...
            if error_code in ['NotAuthorizedException', 'UserNotFoundException']:
                logger.error(f"Authentication failed for user {username}: {e.response['Error']['Message']} : Code {error_code}")
    ...
    def get_user_by_id(self, user_id: str) -> User:
        """
        Get user by their ID (sub attribute)
        ...
        try:
            ...
            return User(
                ...
                updated_at=user_data.get('UserLastModifiedDate'),
